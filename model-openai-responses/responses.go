// Package openairesponses adapts OpenAI Responses HTTP API to
...
const (
	...
	userAgent                = "ingot-model-openai-responses/0.1"
	maxProviderNameBytes     = 64
)

var (
	// ErrInvalidConfig indicates invalid provider configuration.
	ErrInvalidConfig = errors.New("invalid model.openai-responses config")
	...
	// ErrProtocol indicates a malformed or unsupported Responses API response.
...
// ProviderResponseError reports a terminal Responses API object or stream
// event whose status is failed or cancelled.
...
func (e *ProviderResponseError) Error() string {
	return fmt.Sprintf("model provider response %s (%s): %s", e.Status, e.Code, e.Message)
}
...
// Config declares one or more named Responses API providers.
type Config struct {
...
func New(ctx context.Context, deps Dependencies) (Exports, ingotabi.Cleanup, error) {
	if ctx == nil || isNil(deps.HTTP) || isNil(deps.Assets) || isNil(deps.State) {
		return Exports{}, nil, fmt.Errorf("construct model.openai-responses: %w", ErrInvalidConfig)
	}
	...
	if err != nil {
		return Exports{}, nil, fmt.Errorf("construct model.openai-responses: %w: %w", err, ErrInvalidConfig)
	}
	var normalized []normalizedProviderConfig
	if len(cfg.Providers) != 0 {
		normalized, err = normalizeProviders(cfg)
		if err != nil {
			return Exports{}, nil, fmt.Errorf("construct model.openai-responses: %w", err)
		}
	}
...
func (p *provider) Complete(ctx context.Context, request model.Request) (model.Response, error) {
	...
	body, err := p.encodeResponseRequest(ctx, request, false)
	if err != nil {
		return model.Response{}, err
	}
	...
	return decodeResponse(raw, p.name, p.apiKey)
}
