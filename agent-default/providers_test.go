...
func TestNewValidatesSourcesWithoutResolvingDynamicProviders(t *testing.T) {
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			exports, cleanup, err := New(context.Background(), withState(t, Config{Provider: "removed"}, Dependencies{Model: setupProvider{}, Tools: &fakeTools{}, Store: &memoryStore{}, Assets: newMemoryAssets(), Prompt: passthroughPrompt{}, ProviderSources: sources}))
			if test.wantErr {
