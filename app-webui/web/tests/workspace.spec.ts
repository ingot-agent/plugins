...
test('an initially selected complex operation submits the JSON editor contents', async ({ page }) => {
  ...
  await ready(page)
  await openOperationDebugger(page)
...
test('slash command opens a recursive operation dialog without creating a conversation', async ({ page }) => {
  ...
  await composer.fill('/')
  await page.getByRole('option', { name: /\/tool-shell/ }).click()
  await expect(composer).toHaveValue('/tool-shell ')
  await page.getByRole('option', { name: /config/ }).click()
  const dialog = page.getByRole('dialog', { name: '/tool-shell config', exact: true })
  await expect(dialog).toBeVisible()
  await expect(dialog.getByRole('textbox', { name: 'Tool', exact: true })).toHaveCount(0)
...
test('nested providers keep complex detail navigation and edit scalar lists inline', async ({ page }) => {
  ...
  page.on('pageerror', error => errors.push(error.message))
  await ready(page)
  await page.getByRole('textbox', { name: 'Message your agent…', exact: true }).fill('/model-openai-compatible config')
  await page.getByRole('textbox', { name: 'Message your agent…', exact: true }).press('Enter')
  const dialog = page.getByRole('dialog', { name: '/model-openai-compatible config', exact: true })
  await expect(dialog.getByRole('button', { name: /Providers/ })).toBeVisible()
  const width = (await dialog.locator('.field-page').boundingBox())!.width
  await page.screenshot({ path: 'test-results/operation-overview.png' })
  await dialog.getByRole('button', { name: /Providers/ }).click()
  await expect(dialog.getByRole('textbox')).toHaveCount(0)
  await page.screenshot({ path: 'test-results/operation-providers.png' })
  await dialog.getByRole('button', { name: /Example provider/ }).click()
  await expect(dialog.getByRole('textbox', { name: 'Name', exact: true })).toHaveValue('Example provider')
  await dialog.getByRole('textbox', { name: 'Name', exact: true }).fill('Updated provider')
  await expect(dialog.locator('textarea')).toHaveCount(0)
  expect((await dialog.locator('.field-page').boundingBox())!.width).toBeCloseTo(width, 2)
  await page.screenshot({ path: 'test-results/operation-provider-detail.png' })
  const models = dialog.locator('.field-inline-section').filter({ hasText: 'Models' })
  await models.getByRole('button', { name: 'Add item', exact: true }).click()
  await dialog.getByRole('textbox', { name: 'Item 3', exact: true }).fill('example-new')
  await models.getByRole('button', { name: 'Remove item 1', exact: true }).click()
  await expect(dialog.getByRole('textbox', { name: 'Item 1', exact: true })).toHaveValue('example-reasoner')
  await expect(dialog.getByRole('textbox', { name: 'Item 2', exact: true })).toHaveValue('example-new')
  expect((await dialog.locator('.field-page').boundingBox())!.width).toBeCloseTo(width, 2)
  await page.screenshot({ path: 'test-results/operation-models.png' })

  await dialog.getByRole('button', { name: /Default headers/ }).click()
  await dialog.getByRole('button', { name: /X-Tenant/ }).click()
  await expect(dialog.getByRole('textbox', { name: 'Value', exact: true })).toHaveValue('')
  await dialog.getByRole('textbox', { name: 'Name', exact: true }).fill('X-Renamed')
  await dialog.getByRole('radio', { name: 'Replace', exact: true }).check()
  await dialog.getByRole('textbox', { name: 'Value', exact: true }).fill('new-secret')
  expect((await dialog.locator('.field-page').boundingBox())!.width).toBeCloseTo(width, 2)
  await page.screenshot({ path: 'test-results/operation-header-detail.png' })
  await page.setViewportSize({ width: 360, height: 780 })
  expect(await dialog.evaluate(el => el.scrollWidth <= el.clientWidth)).toBe(true)
  expect(await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth)).toBe(true)
  await expect(dialog.getByRole('button', { name: 'Submit response', exact: true })).toBeInViewport()
  const valueBox = (await dialog.getByRole('textbox', { name: 'Value', exact: true }).boundingBox())!
  const footerBox = (await dialog.locator('.overlay-footer').boundingBox())!
  expect(valueBox.y + valueBox.height).toBeLessThanOrEqual(footerBox.y)
  await page.screenshot({ path: 'test-results/operation-header-mobile.png' })
  await expect(dialog.getByRole('textbox', { name: 'Name', exact: true })).toHaveValue('X-Renamed')
  const response = page.waitForRequest(request => request.url().includes('/response') && request.method() === 'POST')
  await dialog.getByRole('button', { name: 'Submit response', exact: true }).click()
  const body = (await response).postDataJSON()
  expect(JSON.stringify(body)).toContain('Updated provider')
  expect(JSON.stringify(body)).toContain('example-new')
  expect(JSON.stringify(body)).toContain('X-Renamed')
  expect(JSON.stringify(body)).toContain('new-secret')
  expect(JSON.stringify(body)).not.toContain('example-chat')
  await expect(dialog.getByText('Operation completed.', { exact: true })).toBeVisible()
  expect(errors).toEqual([])
})

test('operation drilldown works in the debugger and dark Chinese mobile dialog', async ({ page }) => {
  await ready(page)
  await openOperationDebugger(page)
  await page.getByLabel('Select an operation').selectOption({ label: '/model-openai-compatible config' })
  await page.getByRole('button', { name: 'Run operation', exact: true }).click()
  const call = page.locator('.operation-call').first()
  await call.getByRole('button', { name: /Providers/ }).click()
  await call.getByRole('button', { name: /Example provider/ }).click()
  await expect(call.getByRole('textbox', { name: 'Name', exact: true })).toHaveValue('Example provider')
  await expect(page.locator('.operations-view textarea')).toHaveCount(0)
  await call.getByRole('button', { name: 'Submit response', exact: true }).click()
  await page.goto('/#/new')
  await page.getByRole('button', { name: 'Settings', exact: true }).click()
  await page.getByRole('button', { name: 'Dark', exact: true }).click()
  await page.getByLabel('Language', { exact: true }).selectOption('zh')
  await page.getByRole('button', { name: '关闭', exact: true }).click()
  await page.getByRole('textbox', { name: '向你的 Agent 发送消息…', exact: true }).fill('/model-openai-compatible config')
  await page.getByRole('textbox', { name: '向你的 Agent 发送消息…', exact: true }).press('Enter')
  const dialog = page.getByRole('dialog', { name: '/model-openai-compatible config', exact: true })
  await dialog.getByRole('button', { name: /Providers/ }).click()
  await dialog.getByRole('button', { name: /Example provider/ }).click()
  await page.screenshot({ path: 'test-results/operation-provider-dark.png' })
  await page.setViewportSize({ width: 360, height: 780 })
...
test('operation form, complex JSON, result recovery, and cancellation', async ({ page }) => {
  await ready(page)
  await openOperationDebugger(page)
  await page.getByLabel('Select an operation', { exact: true }).selectOption({ label: '/test echo' })
  ...
  await page.reload()
  await expect(page.locator('.operation-call').filter({ hasText: '"value": 42' })).toHaveCount(1)
  await page.getByLabel('Select an operation', { exact: true }).selectOption({ label: '/test complex' })
  ...
  await page.getByLabel('Select an operation', { exact: true }).selectOption({ label: '/test wait' })
  await page.getByLabel('value', { exact: false }).fill('1')
  await page.getByRole('button', { name: 'Run operation', exact: true }).click()
  const call = page.locator('.operation-call').filter({ has: page.getByText('wait', { exact: true }) })
