...
test('nested providers use separate levels with stable width, draft retention and single-line inputs', async ({ page }) => {
  const errors: string[] = []
  page.on('pageerror', error => errors.push(error.message))
  await ready(page)
  ...
  await expect(dialog.locator('textarea')).toHaveCount(0)
  expect((await dialog.locator('.field-page').boundingBox())!.width).toBe(width)
  await page.screenshot({ path: 'test-results/operation-provider-detail.png' })
  await dialog.getByRole('button', { name: /Models/ }).click()
  await expect(dialog.getByRole('textbox', { name: 'Name', exact: true })).toHaveCount(0)
  await dialog.getByRole('button', { name: 'Add item', exact: true }).click()
  await dialog.getByRole('textbox', { name: 'Item 3', exact: true }).fill('example-new')
  await dialog.getByRole('button', { name: 'Remove item 1', exact: true }).click()
  ...
  await page.screenshot({ path: 'test-results/operation-models-mobile.png' })
  await dialog.getByRole('button', { name: 'Back one level', exact: true }).click()
  await expect(dialog.getByRole('textbox', { name: 'Name', exact: true })).toHaveValue('Updated provider')
  await page.screenshot({ path: 'test-results/operation-provider-mobile.png' })
