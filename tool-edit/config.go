...
// state scope, creating the scope when needed. The Host never participates:
// the Operation that triggers a change calls this directly.
...
// normalizeConfig validates the persisted limits and materializes defaults, so
// both construction and the setup Operation apply identical rules.
func normalizeConfig(cfg Config) (normalizedConfig, error) {
	...
	if maxFileBytes < 1 {
		return normalizedConfig{}, fmt.Errorf("max_file_bytes must be positive: %w", ErrInvalidConfig)
	}
	...
	if maxScanBytes < 1 {
		return normalizedConfig{}, fmt.Errorf("max_scan_bytes must be positive: %w", ErrInvalidConfig)
	}
	return normalizedConfig{maxFileBytes: maxFileBytes, maxScanBytes: maxScanBytes}, nil
}
